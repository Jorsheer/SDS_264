...
We are finding the min and max without the NA values, so if there was a row containing an NA that had a min or max in it, it is not considered and thus can throw off the true min and max of the data.
...
the weights in this case are "recycled" in this case, and this is due to the length of the weights not being the same length of the actual values. Hence, we are giving weights to 4,5,6 when we are not dividing by 4,5, and 6 for weights.
...
call. = FALSE in this case would just show the error message we provide instead of tracing back to the error from the function. If we were to have call. = TRUE then we get both error messages.
...
Going through the vector of x, if it contains both true and false statements, it will give each value through x. If we add sum, however, we can then look through the entire vector, and looks to see if there is at least one true value in the vector to then enter the true if statement.
...
The code will produce a dataframe that shows the proportion and instances of a count of a given argument, var, given certain conditions, condition. We have sort = sort since otherwise it sorts by it's natural setting of alphabetically. We do not embrace the df since it is already an env-variable and not masked, we also do not need to create n since it is created through the function "count" and can be referenced later.
...
We create a new column for the different amount of cylinders and rows based on the manufacturer and model. We then sort based on the names alphabetically (default) and create a data frame with this information.
```{r}
#| eval: FALSE
...
```{r}
proportions_sum <- function(x) {
  (x/sum(x, na.rm = TRUE))
}

proportions_sum(temp0)
proportions_sum(temp)
```
...
```{r}
flights |>
  summarize(mean_arrival = mean(arr_delay),
            se_arrival = SE_var(arr_delay))
```

We get the NA's due to the se function not being able to handle this information. We need to modify our se function.
...
test_y <- c(NA, 1, NA, 3, NA)
test_x <- c(1, 2,NA, NA, NA)
...
flights|>
  summarize(
    na_arr = sum(is.na(arr_delay)),
    na_dep = sum(is.na(dep_delay)),
    na_both = both_na(arr_delay, dep_delay)
  )
```

No, it is not necessary since we have 8,255 instances of dep_delay having an na value, and 8,255 that both have it, so if we just get rid of the arr_delay that have have na values, we can then incorporate all of the instances of na's in departure delay.
...
square_inch_from_cm <- function(lengthcm, widthcm) {
  (lengthcm / 2.54) * (widthcm / 2.54)
}


non_answer_convert <- function(x) {
...
vector_ex <- c(6,5,4,3,20)
foo1(vector_ex)

vector_ex <- c(6,1,4,3,20)
foo1(vector_ex)
```

This function sums all the elements that are greater than the next element in the vector and displays how many are greater than the one next to it.
...
```{r}
summary6 <- function(data, var, var2, remove = TRUE) {
  data |> 
    group_by({{ var2 }}) |>
    summarize(
    mean = mean({{ var }}, na.rm = remove),
    median = median({{ var }}, na.rm = remove),
    sd = sd({{ var }}, na.rm = remove),
    IQR = IQR({{ var }}, na.rm = remove),
    n = n(),
    n_miss = sum(is.na({{ var }})),
    .groups = "drop"    # to leave the data in an ungrouped state
  )
...
mpg |> summary6(hwy, drv)
gss_cat|> summary6(age, partyid)
```
...
```{r}
get_last_element <- function(vector){
  vector[length(vector)]
}

vector1 <- c(1,2,3,4,5)
get_last_element(vector1)
```
...
saved_table <- gss_cat|> summary6(age, partyid)

library(ggrepel)

plot_sd_vs_mean <- function(party_age){
  party_age |>
