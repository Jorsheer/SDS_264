...
sentiment_colors <- ifelse(sentiments_20$sentiment == "positive", "green", "red")

wordcloud(
  words = sentiments_20$word, 
  freq = sentiments_20$n, 
  max.words = 100,
...
potter_tidy |>
  ...
  ggplot(aes(x = chapter, y = avg_sentiment)) +
    geom_point(aes(color = title), show.legend = FALSE) +
    facet_wrap(~title, ncol = 2, scales = "free_x")
```
...
```{r}
potter_word_count <- potter_tidy |>
  count(word, title, sort = TRUE)

potter_tidy_idf <- potter_word_count |>
  bind_tf_idf(word, title, n)
...
10 and onward are giving me memory problems. I do not know what is wrong, but I think the code should hypothetically work.

```{r}
#| eval: FALSE

potter_section_words <- potter_untidy |>
  mutate(section = row_number() %/% 80) |> 
  filter(section > 0) |>
  unnest_tokens(word, text) |> 
  filter(!word %in% stop_words$word,
         !word %in% potter_names,
         !word %in% potter_spells,
         !is.na(word))
...
word_cors |>
  filter(item1 %in% c("place", "boy", "wizard", "fly")) |>
  group_by(item1) |>
  slice_max(correlation, n = 6) |>
  ungroup() |>
  mutate(item2 = reorder(item2, correlation)) |>
...
library(tm)
three_books_dtm <- potter_word_count |>
  filter(!word %in% stop_words$word,
         !is.na(word)) |>
  cast_dtm(title, word, n)
