...
typeof(str_view(spot_smaller$subgenre, "pop", match = NA))
class(str_view(spot_smaller$subgenre, "pop", match = NA))

str_view(spot_smaller$subgenre, "pop", html = TRUE)
typeof(str_view(spot_smaller$subgenre, "pop", html = TRUE))
class(str_view(spot_smaller$subgenre, "pop", html = TRUE))

str_subset(spot_smaller$subgenre, "pop")
typeof(str_subset(spot_smaller$subgenre, "pop"))
class(str_subset(spot_smaller$subgenre, "pop"))

str_detect(spot_smaller$subgenre, "pop")
typeof(str_detect(spot_smaller$subgenre, "pop"))
class(str_detect(spot_smaller$subgenre, "pop"))
```
...
```{r}
str_view(bigspotify$track_name, "\\$[0-9]")
```

There are two tracks that include a dollar amount
## Repetition
...
```{r}
str_view(" ", "^.*$") #starts with anything, repeated, and ends
str_view("{example}", "\\{.+\\}") #anything in brackets
```
...
```{r}
str_view(stringr::words, "^[^aeiou]{3}.*([aeiou][^aeiou]){2,}")
```
...
```{r}
spot_smaller |>
  mutate(first_word = str_extract(spot_smaller$title, "^\\b[^ ]+\\b"))
```
...
bigspotify |>
  ...
  select(playlist_name,playlist_decade) |>
...
```{r}
str_view(stringr::words, "([a-z][a-z])(.*)\\1") #[a-z] ensures no numbers
```
...
spot_smaller |>
  select(album_release_date) |>
  mutate(album_release_date = str_replace(album_release_date,"(....)-(..)-(..)", "\\2-\\3-\\1"))
```

17. BEFORE RUNNING IT, explain to your partner(s) what the following R chunk will do:

This code switches the 2nd and 3rd word of the sentence.
