...
gap1952 <- read_excel("1952.xlsx")
gap1957 <- read_excel("1957.xlsx")
```
...
```{r}
paths <- list.files(, pattern = "[.]xlsx$", full.names = TRUE)
paths
```
...
```{r}
vector_trues <- logical(length = numsims)
